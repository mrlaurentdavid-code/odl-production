-- Migration: Save Calculation Results to Database
-- Date: 2025-10-27
-- Purpose: Insert validation results into offer_item_calculated_costs table
--
-- Previously: Function only returned JSON, no database storage
-- Now: Save all calculations for tracking and history
...
COMMENT ON FUNCTION validate_and_calculate_item(UUID, UUID, JSONB) IS 'Validates supplier item, calculates costs, and saves to database - v36';
