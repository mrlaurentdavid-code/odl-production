-- Migration: Smart category/subcategory detection
-- Date: 2025-10-26
-- Issue: WeWeb sends category/subcategory IDs in the "name" fields instead of "id" fields
-- Solution: Detect if "name" field looks like an ID (c1, s5) and use it as ID
...
BEGIN
  ...
  -- If category_name looks like an ID (starts with 'c' followed by digits), treat it as ID
  IF v_category_id IS NULL AND v_category_name ~ '^c[0-9]+$' THEN
    v_category_id := v_category_name;
    v_category_name := NULL;  -- Will be looked up
  END IF;

  -- If subcategory_name looks like an ID (starts with 's' followed by digits), treat it as ID
  IF v_subcategory_id IS NULL AND v_subcategory_name ~ '^s[0-9]+$' THEN
    v_subcategory_id := v_subcategory_name;
    v_subcategory_name := NULL;  -- Will be looked up
  END IF;

  -- Lookup category/subcategory names from IDs if provided
...
'Validates supplier item offer. SMART DETECTION: Detects if category_name/subcategory_name are actually IDs (c1, s5) and treats them as such. Returns category/subcategory details in response.';
