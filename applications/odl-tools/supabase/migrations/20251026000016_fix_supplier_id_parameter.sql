-- Migration: Fix supplier_id to use parameter instead of JSONB extraction
-- Date: 2025-10-26
-- Issue: Function tries to extract supplier_id from JSONB but route handler doesn't include it
-- Solution: Use p_supplier_id parameter directly (comes from API key verification)
...
BEGIN
  ...
  INSERT INTO offer_item_calculated_costs (
    offer_id, item_id, ean, product_name, category_id, category_name,
    ...
    v_offer_id, v_item_id_uuid, v_ean, v_product_name, v_category_id, v_category_name,
...
'Validates supplier item offer. Uses p_supplier_id parameter from API key. Accepts category_id/subcategory_id and looks up names. Auto-generates UUID for item_id when NULL. Returns generated_item_id in response.';
