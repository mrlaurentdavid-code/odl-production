-- Migration: Fix currency rate lookup
-- Date: 2025-10-27
-- Purpose: Fix the currency_rates table query to use correct columns
--
-- The currency_rates table has:
--   - from_currency (TEXT) - e.g. 'EUR', 'USD'
--   - to_currency (TEXT) - e.g. 'CHF'
--   - rate (NUMERIC) - the exchange rate
--
-- NOT: currency_code, chf_rate
...
BEGIN
  ...
  -- FIX: Get currency rate using correct columns
  IF v_purchase_currency != 'CHF' THEN
    ...
    -- Convert to CHF: multiply by rate (if EUR cost 100 and rate is 0.92, CHF cost = 100 * 0.92 = 92)
...
COMMENT ON FUNCTION validate_and_calculate_item(UUID, UUID, JSONB) IS 'Validates supplier item and calculates all costs, margins, and deal status - v34 with correct currency lookup';
