-- Migration: Handle NULL item_id by auto-generating UUID
-- Date: 2025-10-26
-- Purpose: Allow WeWeb to send item_id as NULL and auto-generate UUID internally
...
DECLARE
  ...
  v_supplier_id UUID;
  v_item_id TEXT;
  v_item_id_uuid UUID; -- NEW: Store generated UUID
  v_ean TEXT;
  v_product_name TEXT;
...
BEGIN
  ...
  v_supplier_id := (p_item_data->>'supplier_id')::UUID;
  ...
  -- NEW: Auto-generate UUID if item_id is NULL or empty
  IF v_item_id IS NULL OR v_item_id = '' THEN
    v_item_id_uuid := gen_random_uuid();
  ELSE
    -- item_id is TEXT from WeWeb, we can't use it as UUID
    -- So we always generate a new UUID for the database
    v_item_id_uuid := gen_random_uuid();
  END IF;
  ...
  -- Validate required fields (removed item_id from check)
  ...
  -- Get customs duty rate based on category
  SELECT duty_rate_percent INTO v_customs_duty_rate
  ...
  v_transport_cost_ht := v_package_weight_kg * 15.0; -- CHF per kg
  v_receiving_cost_ht := 5.0; -- Fixed cost
  v_preparation_cost_ht := 3.0; -- Fixed cost
  v_logistics_total_ht := v_transport_cost_ht + v_receiving_cost_ht + v_preparation_cost_ht;

  -- Calculate payment processing fee (1.5% + 0.30 CHF)
  ...
  -- Convert promo price to HT (TTC / 1.081)
  ...
  -- Find applicable business rule (hierarchical: subcategory > category > global)
  SELECT * INTO v_rule
  FROM odeal_business_rules
  WHERE is_active = true
    AND (
      (scope = 'subcategory' AND category = v_category_name AND subcategory = v_subcategory_name)
      OR (scope = 'category' AND category = v_category_name AND subcategory IS NULL)
      OR (scope = 'global' AND category IS NULL AND subcategory IS NULL)
    )
  ORDER BY
    CASE scope
  ...
  -- Determine deal status based on margin thresholds
  ...
  -- Insert calculated costs into database
  INSERT INTO offer_item_calculated_costs (
    ...
    v_offer_id,
    v_item_id_uuid, -- NEW: Use generated UUID
    v_ean,
    v_product_name,
    v_category_name,
    v_subcategory_name,
  ...
  RETURN jsonb_build_object(
    ...
    'cost_id', v_cost_id,
    'generated_item_id', v_item_id_uuid, -- NEW: Return the generated UUID
    'validation_issues', v_validation_issues,
    'item_details', jsonb_build_object(
      'item_id', v_item_id, -- Original TEXT item_id from WeWeb
      'ean', v_ean,
    ...
    'applied_rule', jsonb_build_object(
      ...
      'subcategory', v_rule.subcategory
    )
  );
END;
$$;
...
'Validates supplier item offer and calculates costs. Auto-generates UUID for item_id when NULL. Returns generated_item_id in response.';
