-- Migration: Fix currency_rates column reference
-- Date: 2025-10-26
-- Purpose: Remove reference to non-existent effective_date column

-- Fix validate_and_calculate_item function
...
BEGIN
  ...
  -- Get customs duty rate based on subcategory_id (if provided) or category_name
  IF v_subcategory_id IS NOT NULL AND v_subcategory_id != '' THEN
...
'Validates supplier item offer. Accepts category_id/subcategory_id and looks up names. Auto-generates UUID for item_id when NULL. Returns generated_item_id in response. Fixed: removed effective_date reference from currency_rates lookup.';
